...
public class EnchantmentElbowing extends Enchantment {
    public EnchantmentElbowing() {
        super(Rarity.COMMON, EnumEnchantmentType.WEAPON, new EntityEquipmentSlot[]{EntityEquipmentSlot.MAINHAND});
        this.setRegistryName("elbowing");
        this.setName("elbowing");
        MinecraftForge.EVENT_BUS.register(this);
    }

    @Override
    public boolean canApplyTogether(Enchantment ench) {
        return ench != this && ench != KEnchantmentList.furious && ench != KEnchantmentList.sedation && ench != Enchantments.SHARPNESS;
    }
    ...
    @SubscribeEvent
    public void BoostDashingAttack(LivingHurtEvent event) {
        ...
        if (enchLev > 0)
            if (player.isSprinting()) {
                event.setAmount(event.getAmount() + enchLev * 1.5F);
                for (int i = 0; i < 5; ++i) {
                ...
                event.setAmount(Math.min(0, event.getAmount() - enchLev * 0.25F));
            }
    }
}
